...
const AddToCart = (props) => {
    const { user, product, anchorEl, open, setOpen } = props;
    ...
    const handleAdd = (e) => {
        ...
        if (currentUrl.includes('electronics')) {
            for (let i = 0; i < quantity; i++) {
                addToCartElectronics(user.id, product.id);
            }
        }
        else if (currentUrl.includes('essentials')) {
            for (let i = 0; i < quantity; i++) {
                addToCartEssentials(user.id, product.id);
            }
        }
        ...
        else if (currentUrl.includes('homegoods')) {
            for (let i = 0; i < quantity; i++) {
                addToCartEssentials(user.id, product.id);
            }
        }
        else if (currentUrl.includes('lighting')) {
            for (let i = 0; i < quantity; i++) {
                addToCartEssentials(user.id, product.id);
            }
        }
        else if (currentUrl.includes('pets')) {
            for (let i = 0; i < quantity; i++) {
                addToCartEssentials(user.id, product.id);
            }

        }
        //   essentials, homegoods, grocery, lighting, pets
